...
# 调用API接口运行智能体并返回大模型结果
def invoke_agent(user_id: str, session_id: str, query: str, system_message: str = None):
    """
    调用智能体处理查询，并等待完成
    ...
    while True:
        try:
            ...
            if has_active_session and session_status:
                # 如果是完成或错误状态，自动开始新查询
                if session_status["status"] in ["completed", "error"]:
                    console.print("[info]上次会话已完成或出错，自动开始新查询...[/info]")
                    has_active_session = False
                    session_status = None

            # 获取用户查询
            console.print("[info]请输入您的问题[/info]")
            console.print("[info]命令提示: 'exit' 退出 | 'status' 查询状态 | 'new' 开始新会话 | 'history' 恢复历史会话 | 'setting' 偏好设置 | 'info' 查询基础信息[/info]")
