...
class RedisSessionManager:
    ...
    async def get_user_active_session_id(self, user_id: str) -> str | None:
        ...
        for session_task in session_tasks:
            # 检查 session_task 格式是否正确
            if not session_task or ":" not in session_task:
                logger.warning(f"发现格式不正确的会话任务条目: {session_task}，跳过")
                continue
            try:
                session_id, task_id = session_task.split(":", 1)
            except ValueError:
                logger.error(f"无法解析会话任务格式: {session_task}，跳过")
                continue
    ...
    async def cleanup_all_tasks(self) -> None:
        ...
        async for user_key in self.redis_client.scan_iter("user_sessions:*"):
            ...
            for session_task in session_tasks:
                ...
                if not session_task or ":" not in session_task:
                    # 格式不正确的条目，直接移除
                    logger.warning(f"发现格式不正确的会话任务条目: {session_task}，将移除")
                ...
                try:
                    ...
                    # 分割失败，记录错误并移除无效条目
                    logger.error(f"无法解析会话任务格式: {session_task}，将移除")
                    await self.redis_client.srem(f"user_sessions:{user_id}", session_task)
                    continue
